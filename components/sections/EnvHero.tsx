...
const STATS = [
  { end: 2,  suffix: '×', u: 'FASTER SURVEYS' },
  { end: 95, suffix: '%', u: 'DATA ACCURACY' },
...
export function EnvHero() {
  ...
  useGSAP(() => {
    ...
    tl.fromTo('.env-sub',  { y: 24, opacity: 0 }, { y: 0, opacity: 1, duration: 0.68, ease: 'power3.out' }, '-=0.52')
    tl.fromTo('.env-stat', { y: 20, opacity: 0, scale: 0.88 }, { y: 0, opacity: 1, scale: 1, duration: 0.55, ease: 'back.out(1.5)', stagger: 0.07 }, '-=0.42')
    tl.fromTo('.env-cta',  { y: 14, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, ease: 'power3.out', stagger: 0.1 }, '-=0.35')
    tl.fromTo('.env-img',  { x: 55, opacity: 0, scale: 0.95 }, { x: 0, opacity: 1, scale: 1, duration: 1.1, ease: 'power4.out' }, '-=1.25')
    ...
    STATS.forEach(({ end, suffix }, i) => {
      ...
      gsap.to(obj, {
        ...
        ease: 'power2.out',
        delay: 1.1 + i * 0.15,
        onUpdate() { el.textContent = (end < 10 ? obj.val.toFixed(1).replace('.0', '') : Math.round(obj.val)) + suffix },
      })
    })
  }, { scope: sectionRef })
  ...
  return (
    <section ref={sectionRef} id="hero" className="relative w-full min-h-screen flex items-center overflow-hidden bg-background" style={{ paddingTop: 68 }}>
      ...
      <Container>
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-14 xl:gap-20 items-center py-20 min-h-[calc(100vh-68px)]">
          ...
          <div className="relative z-10">
            ...
            >
              <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#00d4ff', animation: 'envBlink 2s ease-in-out infinite' }} />
              <span className="font-mono font-bold uppercase tracking-[0.22em]" style={{ fontSize: '0.8125rem', color: '#00d4ff' }}>Environmental Monitoring</span>
            </motion.div>
            ...
            >
              <span className="block overflow-hidden">
                <span className="env-word inline-block" style={{ opacity: 0 }}>Environmental</span>
              </span>
              <span className="block overflow-hidden">
                <span className="env-word inline-block" style={{ opacity: 0 }}>
                  <span style={{ background: 'linear-gradient(135deg, #00d4ff 20%, rgba(0,212,255,0.7) 85%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Monitoring</span>
                </span>
              </span>
              <span className="block overflow-hidden">
                <span className="env-word inline-block" style={{ opacity: 0 }}>Photogrammetry</span>
              </span>
            </h1>

            <p className="env-sub font-sans text-text-muted leading-relaxed mb-9"
              style={{ fontSize: '1.1rem', maxWidth: 490, opacity: 0, lineHeight: 1.72 }}
            >
              Addressing climate change requires many solutions working in parallel. Drones equipped with Environmental Monitoring Photogrammetry capabilities provide efficient, high-quality data collection for better conservation decisions.
            </p>

            <div className="grid grid-cols-3 gap-4 mb-9" style={{ borderTop: '1px solid rgba(0,212,255,0.12)', paddingTop: '1.2rem' }}>
              {STATS.map((s, i) => (
                <div key={s.u} className="env-stat flex flex-col gap-1" style={{ opacity: 0 }}>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Link href="#book-demo"
                className="env-cta inline-flex items-center gap-2.5 px-7 py-3.5 rounded-xl font-sans font-semibold transition-all duration-300 hover:scale-[1.04]"
              ...
              <Link href="#book-demo"
                className="env-cta inline-flex items-center gap-2.5 px-7 py-3.5 rounded-xl font-sans font-medium text-text-muted transition-all duration-300 hover:text-white hover:bg-white/[0.05]"
          ...
          <div className="env-img relative" style={{ opacity: 0 }}>
            ...
            <div className="relative rounded-2xl overflow-hidden" style={{ aspectRatio: '4/3', zIndex: 1 }}>
              <Image
                ...
                alt="ZenaDrone — environmental monitoring and photogrammetry"
                fill
              ...
              <div className="absolute bottom-0 inset-x-0 p-5">
                <div className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl" style={{ background: 'rgba(7,7,15,0.82)', border: '1px solid rgba(0,212,255,0.22)', backdropFilter: 'blur(14px)' }}>
                  <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#00d4ff', animation: 'envBlink 2s ease-in-out infinite' }} />
                  <span className="font-mono font-bold uppercase tracking-[0.15em]" style={{ fontSize: '0.67rem', color: '#00d4ff' }}>ZenaDrone — Conservation Intelligence</span>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div className="w-px h-10" style={{ background: 'linear-gradient(to bottom, rgba(0,212,255,0.7), transparent)', animation: 'envBlink 2.2s ease-in-out infinite' }} />
      </motion.div>
      ...
      <style>{`
        @keyframes envBlink { 0%,100% { opacity:.28; } 50% { opacity:1; } }
      `}</style>
    </section>
  )
}
