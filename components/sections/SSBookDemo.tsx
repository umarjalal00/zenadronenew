...
const ORBS = [
  { x: 20, y: 30, w: 480, h: 480, color: '#00d4ff', op: 0.055, d: 9  },
  { x: 78, y: 65, w: 360, h: 360, color: '#00d4ff', op: 0.035, d: 12 },
  { x: 50, y: 10, w: 300, h: 300, color: '#0099cc', op: 0.03,  d: 7  },
]

const TRUST = ['Rapid Deployment', 'Mission Certified', 'Encrypted Data', 'Global Support']

export function SSBookDemo() {
    ...
    <section ref={sectionRef} id="book-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      ...
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          animation: `ssbdOrb ${o.d}s ease-in-out ${i * 3}s infinite`,
        }} />
      ))}
      ...
      <div className="absolute inset-y-0 w-px pointer-events-none ssbd-beam" style={{
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            {submitted ? (
              ...
              >
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full mb-5" style={{ background: 'rgba(0,212,255,0.12)', border: '2px solid rgba(0,212,255,0.38)' }}>
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our team will contact you within 24 hours to schedule your personalised ZenaDrone surveillance demo.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <SSFormField label="Full Name"     name="name"  type="text"  placeholder="Jane Smith"             value={form.name}  onChange={handleChange} required />
                  <SSFormField label="Email Address" name="email" type="email" placeholder="jane@agency.com"        value={form.email} onChange={handleChange} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <SSFormField label="Organisation" name="company" type="text" placeholder="Security Agency / Force" value={form.company} onChange={handleChange} />
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      style={{ background: 'var(--surface-input)', border: '1px solid rgba(255,255,255,0.08)', fontSize: '0.88rem', color: form.useCase ? '#e0f7ff' : '#4a5568' }}
                    ...
                    >
                      ...
                      {['Perimeter Security', 'Search & Rescue', 'Law Enforcement', 'Event Security', 'Anti-Poaching', 'Traffic Management', 'Forest Fire Monitoring', 'Other'].map(o => (
                ...
                <SSFormTextarea
                  label="Tell us about your surveillance needs"
                  name="message"
                  placeholder="Describe your security requirements, coverage area, and what you'd like to achieve with ZenaDrone surveillance solutions..."
                  value={form.message}
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
                  <motion.button
                    ...
                    style={{ background: 'linear-gradient(135deg, #00d4ff 0%, #0097b8 100%)', color: '#07070f', fontSize: '0.92rem', boxShadow: '0 0 28px rgba(0,212,255,0.35)', opacity: loading ? 0.75 : 1 }}
                  >
                    {loading ? (
                      <>
      ...
      <style>{`
        @keyframes ssbdOrb { 0%,100% { transform:translate(-50%,-50%) scale(1); } 50% { transform:translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}

function SSFormField({ label, name, type, placeholder, value, onChange, required }: {
...
function SSFormTextarea({ label, name, placeholder, value, onChange }: {
