...
const ORBS = [
  ...
  { x: 78, y: 65, w: 360, h: 360, color: '#fbbf24', op: 0.045, d: 12 },
  { x: 50, y: 10, w: 300, h: 300, color: '#34d399', op: 0.035, d: 7  },
]

const TRUST = ['Rapid Deployment', 'Mission Certified', 'Encrypted Data', 'Global Support']

export function PLIBookDemo() {
    ...
    <section ref={sectionRef} id="book-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          animation: `plibdOrb ${o.d}s ease-in-out infinite`,
      ...
      <div className="absolute inset-y-0 w-px pointer-events-none plibd-beam" style={{
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            <div className="absolute top-0 left-0 w-32 h-32 pointer-events-none" style={{
              background: 'radial-gradient(ellipse at top left, rgba(0,212,255,0.1) 0%, transparent 70%)',
            }} />
            <div className="absolute bottom-0 right-0 w-32 h-32 pointer-events-none" style={{
              background: 'radial-gradient(ellipse at bottom right, rgba(251,191,36,0.08) 0%, transparent 70%)',
            }} />
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our team will contact you within 24 hours to schedule your personalised ZenaDrone powerline inspection demo.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <PLIFormField label="Full Name"     name="name"  type="text"  placeholder="Jane Smith"          value={form.name}  onChange={handleChange} required />
                  <PLIFormField label="Email Address" name="email" type="email" placeholder="jane@utility.com"    value={form.email} onChange={handleChange} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <PLIFormField label="Organisation" name="company" type="text" placeholder="Utility / Energy Co." value={form.company} onChange={handleChange} />
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      style={{
                        ...
                        fontSize: '0.88rem', color: form.useCase ? '#e0f7ff' : '#4a5568',
                      }}
                    ...
                    >
                      ...
                      {['Transmission Line Inspection', 'Distribution Network', 'Vegetation Management', 'Emergency / Storm Damage', 'Infrastructure Upgrades', 'Routine Maintenance', 'Other'].map(o => (
                        <option key={o} value={o} style={{ background: '#07070f' }}>{o}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <PLIFormTextarea label="Tell us about your inspection needs" name="message" placeholder="Describe your power line infrastructure, current inspection challenges, and what you'd like to achieve with ZenaDrone..." value={form.message} onChange={handleChange} />
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
                  <motion.button
                    ...
                    style={{
                      background: 'linear-gradient(135deg, #00d4ff 0%, #0097b8 100%)',
      ...
      <style>{`
        @keyframes plibdOrb { 0%,100% { transform:translate(-50%,-50%) scale(1); } 50% { transform:translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}

function PLIFormField({ label, name, type, placeholder, value, onChange, required }: {
...
function PLIFormTextarea({ label, name, placeholder, value, onChange }: {
