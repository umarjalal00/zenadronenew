...
// ─── Background orbs ──────────────────────────────────────────────────────────
const ORBS = [
  { x: 20, y: 30, w: 500, h: 500, color: '#00d4ff', op: 0.065, d: 9 },
  { x: 75, y: 55, w: 420, h: 420, color: '#fbbf24', op: 0.05,  d: 12 },
...
// ─── Section ──────────────────────────────────────────────────────────────────
export function IQSquareCTA() {
  const sectionRef = useRef<HTMLElement>(null)
  const [submitted, setSubmitted] = useState(false)
  const [loading,   setLoading]   = useState(false)
  const [form, setForm] = useState({
    name: '', email: '', company: '', phone: '',
    useCase: 'Commercial Inspections',
    message: '',
  })

  useGSAP(() => {
    gsap.fromTo('.iqs-cta-beam',
      { x: '-100%' },
  ...
  return (
    <section ref={sectionRef} id="iq-square-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      {/* Orbs */}
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          opacity: o.op,
          filter: 'blur(60px)',
          animation: `iqs-cta-orb ${o.d}s ease-in-out infinite`,
      ...
      {/* Animated beam */}
      <div className="absolute inset-y-0 w-px pointer-events-none iqs-cta-beam overflow-hidden" style={{
        background: 'linear-gradient(to bottom, transparent, rgba(0,212,255,0.55) 40%, rgba(0,212,255,0.75) 50%, rgba(0,212,255,0.55) 60%, transparent)',
      ...
      <Container size="lg">
        <FadeIn className="text-center mb-14">
          ...
          <p className="font-sans text-text-muted mx-auto" style={{ fontSize: '1.125rem', maxWidth: 540, lineHeight: 1.7 }}>
            Witness the ZenaDrone IQ Square&apos;s operation up close! Please complete the form, and our team will contact you to schedule a demo.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            {/* Corner glows */}
            <div className="absolute top-0 left-0 w-36 h-36 pointer-events-none" style={{ background: 'radial-gradient(ellipse at top left, rgba(0,212,255,0.1) 0%, transparent 70%)' }} />
            <div className="absolute bottom-0 right-0 w-36 h-36 pointer-events-none" style={{ background: 'radial-gradient(ellipse at bottom right, rgba(251,191,36,0.08) 0%, transparent 70%)' }} />
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our team will contact you within 24 hours to schedule your ZenaDrone IQ Square demo.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <TextareaField
                  ...
                  name="message"
                  placeholder="Describe your use case, industry, and what you'd like to achieve with the ZenaDrone IQ Square..."
                  value={form.message}
                  onChange={handleChange}
                />
        ...
        <FadeIn delay={0.2} className="flex flex-wrap justify-center gap-6 mt-10">
          {['Commercial Ready', 'Defense Certified', 'Global Support', 'Fully Autonomous'].map(item => (
      ...
      <style>{`
        @keyframes iqs-cta-orb { 0%,100% { transform: translate(-50%,-50%) scale(1); } 50% { transform: translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}
