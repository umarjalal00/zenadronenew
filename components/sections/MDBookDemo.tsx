...
const ORBS = [
  { x: 18, y: 28, w: 500, h: 500, op: 0.055, d: 10 },
  { x: 80, y: 62, w: 380, h: 380, op: 0.045, d: 13 },
  { x: 50, y: 8,  w: 320, h: 320, op: 0.035, d: 8  },
]

const TRUST = ['Military-Grade Certified', 'AES-256 Encrypted', 'Global Deployment', 'VTOL Any Terrain']

export function MDBookDemo() {
  const sectionRef = useRef<HTMLElement>(null)
  const [submitted, setSubmitted] = useState(false)
  const [loading,   setLoading]   = useState(false)
  const [form, setForm] = useState({ name: '', email: '', organization: '', useCase: '', message: '' })

  useGSAP(() => {
    gsap.fromTo('.mdbd-beam',
      { x: '-100%' },
      { x: '200%', duration: 4.2, ease: 'none', repeat: -1, delay: 1 }
    )
  }, { scope: sectionRef })
    ...
    <section ref={sectionRef} id="book-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          opacity: o.op, filter: 'blur(65px)',
          animation: `mdbdOrb ${o.d}s ease-in-out infinite`,
      ...
      <div className="absolute inset-y-0 w-px pointer-events-none mdbd-beam" style={{
      ...
      <Container size="lg">
        <FadeIn className="text-center mb-14">
          ...
          <p className="font-sans text-text-muted mx-auto" style={{ fontSize: '1.125rem', maxWidth: 540, lineHeight: 1.7 }}>
            Experience ZenaDrone 1000 military capabilities firsthand. Complete the form and our defense team will contact you to schedule a live demonstration.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            <div className="absolute top-0 left-0 w-32 h-32 pointer-events-none" style={{
              background: 'radial-gradient(ellipse at top left, rgba(0,212,255,0.09) 0%, transparent 70%)',
            }} />
            <div className="absolute bottom-0 right-0 w-32 h-32 pointer-events-none" style={{
              background: 'radial-gradient(ellipse at bottom right, rgba(0,212,255,0.07) 0%, transparent 70%)',
            }} />
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our defense team will contact you within 24 hours to schedule your personalized ZenaDrone 1000 demonstration.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <MDFormField label="Full Name"     name="name"         type="text"  placeholder="Col. Jane Smith"         value={form.name}         onChange={handleChange} required />
                  <MDFormField label="Email Address" name="email"        type="email" placeholder="jane@defense.gov"        value={form.email}        onChange={handleChange} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <MDFormField label="Organization" name="organization" type="text"  placeholder="Ministry of Defense / Agency" value={form.organization} onChange={handleChange} />
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      style={{
                        ...
                        fontSize: '0.88rem', color: form.useCase ? '#e8f8ff' : '#4a5568',
                      }}
                ...
                <MDFormTextarea label="Mission Requirements" name="message" placeholder="Describe your operational context, mission objectives, current capabilities, and what you'd like to achieve with ZenaDrone 1000..." value={form.message} onChange={handleChange} />
      ...
      <style>{`
        @keyframes mdbdOrb { 0%,100% { transform:translate(-50%,-50%) scale(1); } 50% { transform:translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}

function MDFormField({ label, name, type, placeholder, value, onChange, required }: {
...
function MDFormTextarea({ label, name, placeholder, value, onChange }: {
