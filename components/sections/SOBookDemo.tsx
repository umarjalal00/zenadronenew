...
const ORBS = [
  { x: 20, y: 30, w: 480, h: 480, op: 0.055, d: 9  },
  { x: 78, y: 65, w: 360, h: 360, op: 0.04,  d: 12 },
  { x: 50, y: 10, w: 300, h: 300, op: 0.035, d: 7  },
]

const TRUST = ['Rapid Deployment', 'Mission Certified', 'Encrypted Data', 'Global Support']

export function SOBookDemo() {
    ...
    <section ref={sectionRef} id="book-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          animation: `sobdOrb ${o.d}s ease-in-out infinite`,
      ...
      <div className="absolute inset-y-0 w-px pointer-events-none sobd-beam" style={{
      ...
      <Container size="lg">
        <FadeIn className="text-center mb-14">
          <p className="font-mono font-bold uppercase tracking-[0.28em] mb-4" style={{ fontSize: '0.8125rem', color: '#00d4ff' }}>
            Get Started
          </p>
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            <div className="absolute bottom-0 right-0 w-32 h-32 pointer-events-none" style={{ background: 'radial-gradient(ellipse at bottom right, rgba(0,212,255,0.07) 0%, transparent 70%)' }} />
            ...
            {submitted ? (
              ...
              >
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full mb-5" style={{ background: 'rgba(0,212,255,0.12)', border: '2px solid rgba(0,212,255,0.4)' }}>
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our team will contact you within 24 hours to schedule your personalised ZenaDrone service demonstration.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <SOFormField label="Full Name"     name="name"  type="text"  placeholder="Jane Smith"         value={form.name}  onChange={handleChange} required />
                  <SOFormField label="Email Address" name="email" type="email" placeholder="jane@company.com"   value={form.email} onChange={handleChange} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <SOFormField label="Organisation" name="company" type="text" placeholder="Company / Agency" value={form.company} onChange={handleChange} />
                  <div className="flex flex-col gap-1.5">
                    <label className="font-mono font-bold text-text-muted uppercase tracking-[0.18em]" style={{ fontSize: '0.8125rem' }}>Service Type</label>
                    <select
                      ...
                      style={{
                        ...
                        fontSize: '0.88rem', color: form.serviceType ? '#e0f7ff' : '#4a5568',
                      }}
                      onFocus={e => { e.currentTarget.style.borderColor = 'rgba(0,212,255,0.38)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(0,212,255,0.07)' }}
                      onBlur={e  => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.boxShadow = 'none' }}
                    >
                      <option value="" style={{ background: '#07070f' }}>Select service type</option>
                      {['Buy-A-Drone (Ownership)', 'Share-A-Drone (Subscription)', 'Aerial Photography', 'Video Capturing', 'Field Scanning', 'Event Coverage', 'Routine Inspections', 'Data Collection', 'Other'].map(o => (
                        <option key={o} value={o} style={{ background: '#07070f' }}>{o}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <SOFormTextarea label="Tell us about your project" name="message" placeholder="Describe your drone service needs, project scope, frequency (daily/weekly/monthly), and what you'd like to achieve with ZenaDrone..." value={form.message} onChange={handleChange} />
        ...
        <FadeIn delay={0.2} className="flex flex-wrap justify-center gap-6 mt-10">
          {TRUST.map(item => (
            <div key={item} className="flex items-center gap-2">
              <div className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: '#00d4ff', opacity: 0.6 }} />
      ...
      <style>{`
        @keyframes sobdOrb { 0%,100% { transform:translate(-50%,-50%) scale(1); } 50% { transform:translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}

function SOFormField({ label, name, type, placeholder, value, onChange, required }: {
...
function SOFormTextarea({ label, name, placeholder, value, onChange }: {
