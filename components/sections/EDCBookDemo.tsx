...
const ORBS = [
  { x: 20, y: 30, w: 480, h: 480, color: '#00d4ff', op: 0.07, d: 9  },
  { x: 75, y: 65, w: 380, h: 380, color: '#7c3aed', op: 0.06, d: 12 },
  { x: 50, y: 10, w: 300, h: 300, color: '#34d399', op: 0.04, d: 7  },
]

const TRUST = ['Enterprise Ready', 'ISO Compliant', 'Encrypted Data', 'Global Support']

export function EDCBookDemo() {
    ...
    <section ref={sectionRef} id="book-demo" className="relative w-full py-28 md:py-40 overflow-hidden bg-background">
      {ORBS.map((o, i) => (
        <div key={i} className="absolute pointer-events-none" style={{
          ...
          animation: `edcbdOrb ${o.d}s ease-in-out infinite`,
      ...
      {/* Beam */}
      <div className="absolute inset-y-0 w-px pointer-events-none edcbd-beam" style={{
      ...
      <Container size="lg">
        <FadeIn className="text-center mb-14">
          <p className="font-mono font-bold text-primary uppercase tracking-[0.28em] mb-4" style={{ fontSize: '0.8125rem' }}>
            Get Started
          </p>
          ...
          <p className="font-sans text-text-muted mx-auto" style={{ fontSize: '1.125rem', maxWidth: 520, lineHeight: 1.7 }}>
            See how the ZenaDrone IQ Square operates firsthand. Complete the form and our team will contact you to schedule a demo.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <div className="relative p-8 md:p-10 rounded-3xl" style={{
            ...
            <div className="absolute bottom-0 right-0 w-32 h-32 pointer-events-none" style={{
              background: 'radial-gradient(ellipse at bottom right, rgba(124,58,237,0.1) 0%, transparent 70%)',
            }} />
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="font-sans text-text-muted" style={{ fontSize: '0.92rem' }}>
                  Our team will contact you within 24 hours to schedule your personalized ZenaDrone demo.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <FormField label="Full Name"       name="name"    type="text"  placeholder="Jane Smith"          value={form.name}    onChange={handleChange} required />
                  <FormField label="Email Address"   name="email"   type="email" placeholder="jane@company.com"    value={form.email}   onChange={handleChange} required />
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <FormField label="Company / Organization" name="company" type="text" placeholder="Your organization" value={form.company} onChange={handleChange} />
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      style={{
                        ...
                        fontSize: '0.88rem', color: form.useCase ? '#f0f4ff' : '#4a5568',
                      }}
                    ...
                    >
                      <option value="" style={{ background: '#07070f' }}>Select industry</option>
                      {['Agriculture', 'Construction', 'Utilities & Energy', 'Infrastructure', 'Defense & Security', 'Environmental', 'Other'].map(o => (
                        <option key={o} value={o} style={{ background: '#07070f' }}>{o}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <FormTextarea label="Tell us about your mission" name="message" placeholder="Describe your use case and what you'd like to achieve with ZenaDrone data collection..." value={form.message} onChange={handleChange} />
      ...
      <style>{`
        @keyframes edcbdOrb { 0%,100% { transform:translate(-50%,-50%) scale(1); } 50% { transform:translate(-50%,-50%) scale(1.12); } }
      `}</style>
    </section>
  )
}
